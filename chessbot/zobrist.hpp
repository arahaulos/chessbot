...
public:
    ...
    uint64_t get_structure_hash(const board_state &state, player_type_t turn) {
        uint64_t h = 0;

        for (int y = 0; y < BOARD_HEIGHT; y++) {
            for (int x = 0; x < BOARD_HEIGHT; x++) {
                h ^= structure_table_xor[y*BOARD_WIDTH + x][state.get_square(square_index(x,y)).d];
            }
        }
        return h;
    }
    ...
    uint64_t update_structure_hash(uint64_t prev_hash, const board_state &state, chess_move m) {
        uint64_t h = prev_hash;

        h ^= structure_table_xor[m.to.index][state.get_square(m.to).d];
        h ^= structure_table_xor[m.from.index][state.get_square(m.from).d];
        ...
        if (state.get_square(m.from).get_type() == PAWN) {
            ...
            if (m.to.get_y() == 0 || m.to.get_y() == BOARD_HEIGHT-1) {
                ...
                h ^= structure_table_xor[m.to.index][p.d];

                return h;
            } else if (m.to == state.en_passant_square && ((state.flags & EN_PASSANT_AVAILABLE) != 0)) {
                h ^= structure_table_xor[state.en_passant_target_square.index][state.get_square(state.en_passant_target_square).d];
            }
        } else if (state.get_square(m.from).get_type() == KING) {

            if (m.to.get_x() == 2 && m.from.get_x() == 4) {
                h ^= structure_table_xor[m.from.get_y() * BOARD_WIDTH + 0][state.get_square(square_index(0, m.from.get_y())).d];
                h ^= structure_table_xor[m.from.get_y() * BOARD_WIDTH + 3][state.get_square(square_index(0, m.from.get_y())).d];
            } else if (m.to.get_x() == 6 && m.from.get_x() == 4) {
                h ^= structure_table_xor[m.from.get_y() * BOARD_WIDTH + 7][state.get_square(square_index(7, m.from.get_y())).d];
                h ^= structure_table_xor[m.from.get_y() * BOARD_WIDTH + 5][state.get_square(square_index(7, m.from.get_y())).d];
            }
        }

        h ^= structure_table_xor[m.to.index][state.get_square(m.from).d];
...
private:
    ...
    uint64_t structure_table_xor[BOARD_HEIGHT*BOARD_WIDTH][16];
