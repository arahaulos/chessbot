...
alphabeta_search::alphabeta_search(const alphabeta_search &other): move_cache(other.move_cache), eval_cache(other.eval_cache)
{
...
{
    for (uint64_t i = 0; i < move_cache.get_size(); i++) {
        move_cache[i].clear();
    }
}
...
chess_move alphabeta_search::get_move(int pv_num)
{
    chess_move p;
    best_move_lock.lock();
    p = best_pv[pv_num].moves[0];
    best_move_lock.unlock();
    return p;
}

int32_t alphabeta_search::get_evaluation(int pv_num)
{
    int32_t e;
    best_move_lock.lock();
    e = best_pv[pv_num].score;
    best_move_lock.unlock();
    return e;
}

void alphabeta_search::get_pv(pv_table &pv_out, int pv_num)
{
    best_move_lock.lock();
    pv_out = best_pv[pv_num];
    best_move_lock.unlock();
}

int32_t alphabeta_search::get_depth()
{
    best_move_lock.lock();
    int depth = best_move_depth;
    best_move_lock.unlock();
    return depth;
}
...
void alphabeta_search::iterative_search(int max_depth)
{
    ...
    for (int i = 0; i < num_of_pvs; i++) {
        best_pv[i].num_of_moves = 0;
        best_pv[i].score = MIN_EVAL;
    }
    ...
    if (book_move.valid() && use_opening_book) {
        best_move_lock.lock();
        best_move_depth = 1;
        best_pv[0].first(book_move);
        best_pv[0].score = 0;
        best_move_lock.unlock();
        ready_flag = true;
    ...
    if (legal_moves.size() == 1) {
        best_move_lock.lock();
        best_move_depth = 1;
        best_pv[0].first(legal_moves[0]);
        best_pv[0].score = 0;
        best_move_lock.unlock();
        ready_flag = true;
    } else if (legal_moves.size() == 0) {
        std::cout << "Error: cannot search position where is no legal moves" << std::endl;
        return;
    }
    ...
    /*static double filtered_diff = 0.0f;

    if (all_threads_stats.se_diff_samples > 0) {
        double diff = all_threads_stats.avg_se_diff / all_threads_stats.se_diff_samples;
        filtered_diff = filtered_diff*0.95 + diff * 0.05f;
        std::cout << filtered_diff << std::endl;
    }*/
...
void alphabeta_search::run_perft(board_state &state, int depth, bool debug)
{
    if (debug) {
        ...
        std::shared_ptr<nnue_network> nnue = std::make_shared<nnue_network>(shared_nnue_weights);
...
uint64_t alphabeta_search::perft(board_state &state, int depth)
{
    if (depth == 0) {
        return 1;
    }
...
uint64_t alphabeta_search::debug_perft(board_state &state, int depth, int ply, search_context &sc, int32_t &eval_sum)
{
    ...
    move_picker picker;
    picker.init(state, ply, tt_move, chess_move::null_move(), sc.history, experimental_features);
    ...
    while (picker.pick(m) != MOVES_END) {
        ...
        //test_perft_tt.prefetch(hashgen.update_hash(state.zhash, state, m));
    ...
    if (total < 32000) {
        test_perft_tt[state.zhash].write(state.zhash, best_move, depth, ALL_NODE, total, total, 0, current_cache_age);
    }
